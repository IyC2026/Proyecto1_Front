    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <Card className="w-full max-w-2xl bg-white mx-auto shadow-lg rounded-2xl overflow-hidden transform transition-all duration-300 ease-in-out">
        <EncabezadoFormularios
          title={presentacion ? `Actualizar ${NOMBRE_ENTIDAD}` : `Nueva ${NOMBRE_ENTIDAD}`}
          subtitle={
            presentacion
              ? "Modifica los datos del formato comercial."
              : "Ingresa los datos para registrar una nueva Presentación."
          }
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <CardContent className="space-y-3 px-3 py-2">
              <FormInput
                name="denominacion"
                label="Denominación"
                placeholder="Ingresa la denominación (ej: 1.5L, 500ml, Pack x6...)"
